...
<table id="cTableFSRW" class="table table-bordered" width="100%"
	cellspacing="0">
	<thead>
		<tr>
			<th>課程編號</th>
	...
	<tbody>
		<c:forEach var="cSearchResultw" varStatus="stat" items="${psbw}">
			<tr>
				<td>${cSearchResultw.coId}</td>  <!--直接設定編號${stat.count}-->
				<td>${cSearchResultw.coTitle}</td>
				<td>${cSearchResultw.coAct_Type}</td>
				<td><img style='display: block; width: 320px; height: 180px;'
					src="data:image/jpg;base64, ${cSearchResultw.coAct_ImageStr}"></td>
				<td><a href="<c:url value='/18/cCourseDetail.ctrl?coId=${cSearchResultw.coId}' /> "><input class='toDetail' type='button' value='查看課程詳細'/></a></td>
				<td>${cSearchResultw.coHot}</td>
			</tr>
		</c:forEach>
